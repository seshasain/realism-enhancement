...
def main(input_image_path="Asian+Man+1+Before.jpg", detail_amount=0.7, upscale_factor=4):
    import_custom_nodes()
    with torch.inference_mode():
        loadimage = NODE_CLASS_MAPPINGS["LoadImage"]()
        loadimage_1 = loadimage.load_image(image=input_image_path)
        ...
        # Return the output images for the handler
        return {
            "comparison": get_value_at_index(cr_simple_image_compare_74, 0),
            "final_resized": get_value_at_index(imageresizekjv2_185, 0),
            "final_hires": get_value_at_index(ultimatesdupscalecustomsample_194, 0),
            "first_hires": get_value_at_index(ultimatesdupscalecustomsample_178, 0)
        }
