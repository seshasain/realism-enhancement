...
RUN pip install --upgrade pip && \
    ...
    # Pin numpy to 1.x to avoid compatibility issues with torch
...
RUN echo "Build timestamp: 2025-06-28-09:30:00-NUMPY-FIX-LOGGING-ENHANCED"
...
RUN if [ -d "venv" ]; then \
        ...
        venv/bin/pip install --no-cache-dir runpod>=1.5.0 boto3>=1.28.0 "numpy>=1.22.0,<2.0.0" python-json-logger>=2.0.0; \
        echo "✅ RunPod SDK installation completed"; \
    else \
        echo "Installing ComfyUI dependencies"; \
        pip install --no-cache-dir -r requirements.txt runpod>=1.5.0 boto3>=1.28.0 "numpy>=1.22.0,<2.0.0" python-json-logger>=2.0.0; \
    fi
